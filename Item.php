...
$productQuery = mysqli_prepare($conn, $sql);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["submit_review"])) {
    if (!isset($_SESSION["user_id"])) {
        ...
        if ($reviewTitleValue === "" || $reviewTextValue === "" || $reviewRatingValue === "") {
            ...
            // Save each review with the product and the user
            ...
            if (!$insertQuery) {
                ...
                mysqli_stmt_bind_param($insertQuery, "iissi", $id, $userId, $reviewTitleValue, $reviewTextValue, $ratingNumber);
...
// Load the average on its own so the summary can still show
$averageQuery = mysqli_prepare($conn, "SELECT AVG(review_rating) AS average_rating, COUNT(*) AS review_count FROM tbl_reviews WHERE product_id = ?");

if ($averageQuery) {
    mysqli_stmt_bind_param($averageQuery, "i", $id);
    mysqli_stmt_execute($averageQuery);
    $averageResult = mysqli_stmt_get_result($averageQuery);
    $averageRow = mysqli_fetch_assoc($averageResult);
    mysqli_stmt_close($averageQuery);
...
// Pull the user name in with each review
$reviewsQuery = mysqli_prepare(
    $conn,
...
if ($reviewsQuery) {
    mysqli_stmt_bind_param($reviewsQuery, "i", $id);
    mysqli_stmt_execute($reviewsQuery);
    $reviewsResult = mysqli_stmt_get_result($reviewsQuery);

    while ($reviewRow = mysqli_fetch_assoc($reviewsResult)) {
        $reviews[] = $reviewRow;
    }

    mysqli_stmt_close($reviewsQuery);
} else {
    $reviewsAvailable = false;
}
...
echo '    <form action="Item.php?id=' . $id . '" method="POST" class="add-to-cart-form">';
...
if (!empty($reviews)) {
    foreach ($reviews as $review) {
        // Use a fallback name if the join comes back empty
        $reviewerName = "";
        if (isset($review['user_name'])) {
            $reviewerName = trim((string) $review['user_name']);
        }
        if ($reviewerName === '') {
