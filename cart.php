...
function buildCartItemsFromCookie($conn, $cart)
{
    ...
    if (!empty($cart)) {
        // The cookie only stores ids and quantities so titles and prices come from the database
        $idList = implode(",", array_keys($cart));
...
// These session values act like one time messages
...
// Redirect after each cart action so refresh does not repeat it
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST["apply_offer"])) {
        ...
        if ($offerInputValue === "GRAD25") {
            ...
            // Check the code in the database not in the browser
            $offerQuery = mysqli_prepare($conn, "SELECT offer_id FROM tbl_offers WHERE offer_title = ? LIMIT 1");

            if ($offerQuery) {
                mysqli_stmt_bind_param($offerQuery, "s", $offerTitle);
                mysqli_stmt_execute($offerQuery);
                $offerResult = mysqli_stmt_get_result($offerQuery);
    ...
    if (isset($_POST["checkout"])) {
        ...
        // Read the cart again right before checkout
        ...
        $orderQuery = mysqli_prepare($conn, $insertSql);

        if (!$orderQuery) {
            $_SESSION["checkout_error"] = "The orders table needs the expected order columns before checkout can work.";
        } else {
            mysqli_stmt_bind_param($orderQuery, "is", $userId, $cartSummaryText);

            if (mysqli_stmt_execute($orderQuery)) {
                // Only clear the cart after the order is saved
                clearCartCookieData();
                unset($_SESSION["cart_discount"]);
                unset($_SESSION["cart_offer_code"]);
...
// Keep the discount in session while the cart stays the same
$discountRate = 0;
...
if (empty($cartItems)) {
    ...
    foreach ($cartItems as $item) {
        ...
        echo '        <img src="' . $itemImage . '" alt="' . $itemTitle . '" class="cart-image">';
